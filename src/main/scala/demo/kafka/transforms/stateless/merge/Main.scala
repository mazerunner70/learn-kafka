package demo.kafka.transforms.stateless.merge
...
class Main {
  ...
  def topologyPlan(): StreamsBuilder = {
    ...
    branches(0).merge(branches(1)).to("merged")
...
//    fibValues.map((k, v) => (v.last.toString, v)).to("digitcounts2")
  ...
  def run() = {
    ...
    val consumer2 = Consumer[String, String]("grp2", "556")
    val consumer3 = Consumer[String, String]("grp3", "554")
    val consumer4 = Consumer[String, String]("grp4", "559")

    rawConsumer.subscribe("fib", x=>println(s"raw= $x"))
    consumer2.subscribe("branch1", x=>println(s"branch1 = $x"))
    consumer3.subscribe("branch2", x=>println(s"branch2 = $x"))
    consumer4.subscribe("merged", x=>println(s"merged = $x"))
