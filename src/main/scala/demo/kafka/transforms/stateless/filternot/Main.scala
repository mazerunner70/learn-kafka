package demo.kafka.transforms.stateless.filternot
...
class Main {
  ...
  def topologyPlan(): StreamsBuilder = {
    ...
    val filtered = fibValues.filterNot((k, v) => (v.last.toInt-48>4))
    fibValues.to("unfiltered")
...
//    fibValues.map((k, v) => (v.last.toString, v)).to("digitcounts2")
  ...
  def run() = {
    ...
    consumer2.subscribe("unfiltered", x=>println(s"digits = $x"))
    consumer3.subscribe("filtered", x=>println(s"digits2 = $x"))
