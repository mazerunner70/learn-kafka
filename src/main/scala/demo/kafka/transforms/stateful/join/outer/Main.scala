package demo.kafka.transforms.stateful.join.outer
...
class Main {
  ...
  def topologyPlan(): StreamsBuilder = {
    ...
    val joined = filtered1.outerJoin(filtered2)((lv, rv)=>"left=" + lv + ", right=" + rv, JoinWindows.of(Duration.ofSeconds(90L)))
    joined.to("joined")
    builder
  }
