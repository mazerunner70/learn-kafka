package demo.kafka.globalktabletransforms.stateful.join.inner
...
class KStreamMain {
  ...
  def topologyPlan(): StreamsBuilder = {
    ...
    val joined = filtered1.join(filtered2asGlobal)(
      (lk, lv) => {println(lk+"h");lk},
  ...
  def run() = {
    ...
    rawConsumer.subscribe("fib", x=>println(s"fib= $x"))
  ...
  def run1() = {
    ...
    consumer3.subscribe("joined", x=>println(s"joined = $x"), x=>x.recordCounter == 10)
