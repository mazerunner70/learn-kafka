package demo.kafka.ktabletransforms.stateful.aggregation.reduce
...
class Main {
  ...
  def topologyPlan(): StreamsBuilder = {
    ...
    fibValues.toStream.to("unreduced")
    val groupedTable: KGroupedTable[String, String] = fibValues.groupBy((k, v) => ((k.toInt/4).toString, v))
    val counted: KTable[String, String] = groupedTable.reduce(
      (aggValue, newValue)=> s"add '$aggValue', '$newValue''",
      (aggValue, oldValue)=> s"remove '$aggValue', '$oldValue''")
