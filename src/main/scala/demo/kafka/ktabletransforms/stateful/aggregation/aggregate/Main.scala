package demo.kafka.ktabletransforms.stateful.aggregation.aggregate
...
class Main {
  ...
  def topologyPlan(): StreamsBuilder = {
    ...
    fibValues.toStream.to("ungrouped")
    val groupedTable: KGroupedTable[String, String] = fibValues.groupBy((k, v) => (k, ( k.toInt/4).toString))
    val aggregated: KTable[String, String] = groupedTable.aggregate[String]("")((aggKey, newValue, aggValue) => {
      println(s"$aggKey, $newValue, $aggValue")
      "add "+ aggValue + ',' + newValue
    }, (aggKey, newValue, aggValue) => {
      println(s"$aggKey, $newValue, $aggValue")
      "minus " + aggValue + ',' + newValue
    })
  ...
  def run() = {
    ...
    consumer2.subscribe("ungrouped", x=>println(s"ungrouped = $x"))
